...
(function () {
  ...
  function openPartnerDashboard() {
    ...
    <style>
        ...
        :root {
            ...
            --text-sub: #475569;
            --border: #e2e8f0;
            --primary: #6366f1; /* Daha canlı İndigo */
        ...
        .p-overlay { 
            ...
            font-family: 'Inter', sans-serif; opacity: 0; animation: fadeIn 0.3s forwards;
        }

        .p-app-container {
            width: 95%; max-width: 1200px; height: 90vh; max-height: 900px;
            ...
            transform: scale(0.98); animation: zoomIn 0.3s 0.1s forwards; color: var(--text-main);
        }
        ...
        .p-logo-icon {
            width: 40px; height: 40px; background: linear-gradient(135deg, var(--primary), #818cf8);
            color: white; border-radius: 10px; display: flex; align-items: center; justify-content: center;
            font-size: 20px; box-shadow: 0 10px 20px -5px rgba(79, 70, 229, 0.5);
        }
        .p-logo-text { font-weight: 800; font-size: 18px; letter-spacing: -0.5px; }

        /* BUTONLAR (NET VE BELİRGİN) */
        .p-nav-btn {
            ...
            background: transparent; /* Varsayılan */
        ...
        .p-nav-btn i { width: 20px; text-align: center; font-size: 16px; }

        .p-user-card {
            margin-top: auto; display: flex; align-items: center; gap: 12px;
            padding: 14px; background: var(--bg-body); border-radius: 16px; border: 1px solid var(--border);
        }
        .p-avatar {
            width: 40px; height: 40px; background: var(--accent); color: white;
            border-radius: 10px; display: flex; align-items: center; justify-content: center;
            font-weight: 800; font-size: 16px; box-shadow: 0 4px 12px rgba(245, 158, 11, 0.3);
        }

        /* --- SAĞ İÇERİK (MAIN) --- */
        .p-main { flex: 1; display: flex; flex-direction: column; overflow: hidden; position: relative; }
        
        .p-header {
            height: 80px; display: flex; align-items: center; justify-content: space-between;
            padding: 0 30px; border-bottom: 1px solid var(--border); background: var(--glass-bg);
            backdrop-filter: blur(20px); position: sticky; top: 0; z-index: 40;
        }
        .p-page-title { font-size: 22px; font-weight: 800; color: var(--text-main); }
        
        /* BURGER MENU BUTONU (Masaüstünde gizli) */
        .p-burger-btn {
            display: none; /* Masaüstünde gizle */
            width: 40px; height: 40px; align-items: center; justify-content: center;
            background: var(--bg-body); border-radius: 10px; border: 1px solid var(--border);
            color: var(--text-main); font-size: 20px; cursor: pointer; margin-right: 15px;
        }
        ...
        /* --- MOBİL UYUM (PREMIUM BURGER MENÜ) --- */
        @media (max-width: 768px) {
            .p-app-container { width: 100%; height: 100%; border-radius: 0; border: none; }
            
            /* Sidebar'ı Gizle (Ekrandan dışarı it) */
            .p-sidebar { 
                position: absolute; top: 0; left: 0; height: 100%; width: 80%; max-width: 300px;
                transform: translateX(-100%); /* Gizli */
                box-shadow: 5px 0 30px rgba(0,0,0,0.5); /* Açılınca gölge at */
            }
            /* Sidebar Açıkken */
            .p-sidebar.show-mobile {
                transform: translateX(0); /* Göster */
            }
            
            /* Menü açılınca arkaya karartma katmanı (Opsiyonel ama iyi olur) */
            .p-sidebar-overlay {
                display: none; position: absolute; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:45;
            }
            .p-sidebar.show-mobile + .p-sidebar-overlay { display: block; }

            .p-burger-btn { display: flex; } /* Mobilde göster */
            .p-header { padding: 0 20px; height: 70px; }
            .p-page-title { font-size: 18px; }
            .p-content { padding: 20px; }
            
            /* Kart düzenlerini mobilde tek sütuna düşür */
    ...
    <div id="mdm-partner-modal" class="p-overlay">
        ...
        <div class="p-app-container">
            <div class="p-sidebar" id="p-mobile-sidebar">
                <div class="p-logo-area">
                    <div class="p-logo-icon">M</div>
                    <div>
                        <div class="p-logo-text">Partner Pro</div>
                        <div class="p-logo-sub" style="font-size:11px; color:var(--text-sub);">Panel v3.2</div>
                    </div>
                    <div class="hide-desktop show-mobile-flex" onclick="PartnerApp.toggleMobileMenu()" style="margin-left:auto; padding:10px; cursor:pointer;">
                        <i class="fas fa-times"></i>
                    </div>
                </div>

                <div class="p-nav-btn active" onclick="PartnerApp.loadTab('home', this); PartnerApp.toggleMobileMenu()">
                    <i class="fas fa-home"></i> <span>Genel Bakış</span>
                </div>
                <div class="p-nav-btn" onclick="PartnerApp.loadTab('links', this); PartnerApp.toggleMobileMenu()">
                    <i class="fas fa-link"></i> <span>Link Oluştur</span>
                </div>
                <div class="p-nav-btn" onclick="PartnerApp.loadTab('showcase', this); PartnerApp.toggleMobileMenu()">
                    <i class="fas fa-fire"></i> <span>Fırsat Vitrini</span>
                </div>
                <div class="p-nav-btn" onclick="PartnerApp.loadTab('wallet', this); PartnerApp.toggleMobileMenu()">
                    <i class="fas fa-wallet"></i> <span>Cüzdan</span>
                </div>
                <div class="p-nav-btn" onclick="PartnerApp.loadTab('marketing', this); PartnerApp.toggleMobileMenu()">
                    <i class="fas fa-images"></i> <span>Medya Kiti</span>
                </div>
                <div class="p-nav-btn" onclick="PartnerApp.loadTab('profile', this); PartnerApp.toggleMobileMenu()">
                    <i class="fas fa-user-cog"></i> <span>Profil & Ayar</span>
                </div>

                <div class="p-user-card">
                    <div class="p-avatar">${avatarInitial}</div>
                    <div style="overflow:hidden;">
                        <div style="font-weight:800; font-size:14px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${name}</div>
                        <div style="font-size:12px; color:var(--text-sub); font-weight:500;">Aktif Ortak</div>
                    </div>
                    </div>
            </div>
            
            <div class="p-sidebar-overlay" onclick="PartnerApp.toggleMobileMenu()"></div>

            <div class="p-main">
                <div class="p-header">
                    <div style="display:flex; align-items:center;">
                        <div class="p-burger-btn" onclick="PartnerApp.toggleMobileMenu()">
                            <i class="fas fa-bars"></i>
                        </div>
                        <div class="p-page-title" id="p-header-title">Genel Bakış</div>
                    </div>
                    
                    <div style="display:flex; gap:12px;">
                        <div onclick="PartnerApp.showTierInfo()" style="cursor:pointer; background:var(--bg-card); padding:10px 16px; border-radius:12px; font-size:13px; font-weight:700; color:var(--primary); border:1px solid var(--border); display:flex; align-items:center; gap:8px; box-shadow:var(--shadow-md);">
                             <i class="fas fa-trophy"></i> <span class="hide-mobile">Oranlar</span>
                        </div>
                        <div onclick="document.getElementById('mdm-partner-modal').remove()" style="cursor:pointer; width:44px; height:44px; display:flex; align-items:center; justify-content:center; border-radius:12px; background:var(--bg-card); color:var(--text-sub); border:1px solid var(--border); box-shadow:var(--shadow-md); font-size:18px; transition:all 0.2s;">
                            <i class="fas fa-times"></i>
                        </div>
                    </div>
                </div>

                <div id="p-content-area" class="p-content">
                    </div>
            </div>
        </div>
    </div>
    ...
    // İlk açılışta Home sekmesini yükle
    var homeBtn = document.querySelector(".p-nav-btn");
  ...
  window.PartnerApp = {
    ...
    }, // --- YENİ: MOBİL MENÜYÜ AÇ/KAPA ---
    toggleMobileMenu: function () {
      const sidebar = document.getElementById("p-mobile-sidebar");
      if (sidebar) {
    ...
    loadTab: function (tab, el) {
      ...
      const titles = {
        home: "Genel Bakış",
        links: "Link Oluşturucu",
        showcase: "Günün Vitrini",
        wallet: "Cüzdan & Finans",
        marketing: "Pazarlama Kiti",
        profile: "Profil Ayarları",
      };
      const titleEl = document.getElementById("p-header-title");
      if (titleEl) titleEl.innerText = titles[tab] || "Panel";

      var area = document.getElementById("p-content-area");

      // 🔥 SKELETON LOADING (PROFESYONEL YÜKLENİYOR EFEKTİ)
      area.innerHTML = `
      ...
      // Kısa bir gecikme ile gerçek içeriği yükle (Geçiş hissi için)
      setTimeout(() => {
